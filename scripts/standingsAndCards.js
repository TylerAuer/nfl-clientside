    ...
    while (unrankedParticipantsList.length > 0) {
      ...
      while (lowestScoringParticipantsLeft.length > 1) {
        ...
        for (participant of lowestScoringParticipantsLeft) {
          tiebreakerScore.push(
            parseInt(standings[participant][tiebreakerLevel][2])
          );
        }
        ...
        for (var i = 0; i < lowestScoringParticipantsLeft.length; i++) {
          if (
            standings[lowestScoringParticipantsLeft[i]][tiebreakerLevel][2] >
            minTiebreakerScore
          ) {
      ...
      standings[lowestScoringParticipantsLeft[0]][0][0] =
        unrankedParticipantsList.length;
        ...
        if (standings[participant][0][0] == i) {
          ...
          for (pick in standings[participant]) {
            ...
            if (standings[participant][pick][1] == "W") {
              cell4.className +=
                " tableWin align-middle d-none d-md-table-cell";
            } else {
              cell4.className +=
                " tableLoss align-middle d-none d-md-table-cell";
            }
  ...
  request.onload = function() {
    ...
    if (request.status >= 200 && request.status < 400) {
      ...
      data.gameScores.forEach(game => {
        ...
        var posIcon =
          '<img src="img/posIcon.png" alt="Possession Icon" class="posIcon">';
        ...
        if (gameWinner == awayTeam) {
          awayTeamLine1HTML += wrapInTag(
            awayTeam + " " + awayTeamScore,
            "span",
            'class="gameCardAwayTeamHappy";'
          );

          if (awayTeamWinsOwner != null) {
            awayTeamLine2HTML += wrapInTag(
              "<b>W: </b>" + awayTeamWinsOwner,
              "span",
              'class="gameCardOwnerHappy";'
            );
            awayTeamLine2HTML += " ";
          }
          if (awayTeamLossesOwner != null) {
            awayTeamLine2HTML += wrapInTag(
              "<b>L: </b>" + awayTeamLossesOwner,
              "span",
              'class="gameCardOwnerSad";'
            );
          }
        ...
        var awayScoreHTML = wrapInTag(
          awayTeamScore,
          "span",
          'class="gameCardAwayTeamScore";'
        );
        ...
        if (gameWinner == homeTeam) {
          homeTeamLine1HTML += wrapInTag(
            homeTeam + " " + homeTeamScore,
            "span",
            'class="gameCardHomeTeamHappy";'
          );

          if (homeTeamWinsOwner != null) {
            homeTeamLine2HTML += wrapInTag(
              "<b>W: </b>" + homeTeamWinsOwner,
              "span",
              'class="gameCardOwnerHappy";'
            );
            homeTeamLine2HTML += " ";
        ...
        var homeScoreHTML = wrapInTag(
          homeTeamScore,
          "span",
          'class="gameCardHomeTeamScore";'
        );
        ...
        switch (gameState) {
          ...
          case "OT5": // Overtime
            details.innerHTML +=
              down + " and " + distance + " | " + qtrTimeLeft + " OT";
      ...
      console.log(
        "API request of https://feeds.nfl.com/feeds-rs/scores.json failed."
      );
    }
  };
